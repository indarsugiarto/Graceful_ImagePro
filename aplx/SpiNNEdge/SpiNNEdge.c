...
void hMCPL(uint key, uint payload)
{
    ...
    else if(key==MCPL_BCAST_CMD_SEND_RESULT) {
        if(payload==blkInfo->nodeBlockID-1)
            sendResult(payload+1,0);    // then it's my turn
    }
    // TODO: MCPL_BCAST_HOST_ACK is not working yet!!!
